...
- **A**(进行中):盲推核心。**修正 §9.4 规模估计**——三份 Explore 报告(AnalyzeUrl/AnalyzeRule/BaseSource)核实:真正互递归**只有一个环**(AnalyzeUrl↔ConcurrentRateLimiter↔ConcurrentRecord),抽 `ConcurrentRecord` 为顶层类即断(batch 2a 已断);其余是 import/SPI 改写 + 补 util 缺口,**可分小批增量、各自 CI 绿**,只有 flip 步骤需闭包同批。
  ...
  - ⏭️ batch 2c-4 = 剩余 JsExtensions(闭包后增量,各自 CI 绿):HTTP 簇 ajaxAll/ajaxTestAll(需 mapAsync 移植 + Platform.appConfig.threadCount)、connect/get/post/head(jsoup + SSLHelper + ConcurrentRateLimiter(key,rate));WebView 簇 webView×4/webViewGetSource×3/webViewGetOverrideUrl×3(→Platform.webView);文件/网络 downloadFile×2(AnalyzeUrl + FileUtils + UrlUtil.getSuffix 移植)、importScript/cacheFile×2;压缩 getZip/Rar/7zByteArrayContent + String 包装 + unzip/un7z/unrar/unArchiveFile/getTxtInFolder(需 ArchiveUtils/LibArchiveUtils 移植);字体 queryTTF×3/queryBase64TTF/replaceFont×2(需 QueryTTF + AppCacheManager + toStringArray 移植)。app-only(openVideoPlayer/openUrl/toast/getVerificationCode/startBrowser*/getReadBookConfig/getThemeConfig/androidId)不进引擎。
