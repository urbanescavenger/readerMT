...
class ReplaceRuleController(coroutineContext: CoroutineContext): BaseController(coroutineContext) {
    suspend fun getReplaceRules(context: RoutingContext): ReturnData {
        ...
        if (list == null) {
            // 用户无替换规则,播种默认规则(如 #01 数字标题#JS 章节标题归一化)
            val defaults = DefaultData.replaceRules
            if (defaults.isNotEmpty()) {
                val arr = JsonArray()
                defaults.forEach { arr.add(JsonObject.mapFrom(it)) }
                saveUserStorage(userNameSpace, "replaceRule", arr)
                return returnData.setData(arr.getList())
            }
    ...
    suspend fun getDefaultReplaceRules(context: RoutingContext): ReturnData {
        val returnData = ReturnData()
        checkAuth(context)
        val defaults = DefaultData.replaceRules
        val arr = JsonArray()
