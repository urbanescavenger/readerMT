...
interface JsExtensions {
    ...
    /**
     * 启动浏览器加载页面并等待,返回页面结果(legado 客户端用于人机验证/JS渲染)。
     * 服务端通过 reader.app.remoteWebviewApi 指向的无头浏览器服务(browserless)渲染:
     * 加载 URL、等 CF 的 JS 挑战自动通过、回传页面 HTML + cookies,返回 StrResponse(源里用 r.body() 取 HTML)。
     * 未配置 remoteWebviewApi 时抛清晰错误;交互式 Turnstile(需人点)仍可能失败。
     */
    fun startBrowserAwait(urlStr: String, headerStr: String, tag: Any?): StrResponse {
        val source = getSource()
        return WebViewRenderHelp.renderUrl(urlStr, source?.getKey() ?: urlStr, getWebViewUA())
    }

    fun startBrowserAwait(urlStr: String, headerStr: String): StrResponse =
        startBrowserAwait(urlStr, headerStr, null)

    fun startBrowserAwait(urlStr: String): StrResponse =
        startBrowserAwait(urlStr, "", null)

    /**
     * 启动浏览器(异步),服务端无 GUI 不支持
     */
    fun startBrowser(urlStr: String, headerStr: String, tag: Any?): String {
        throw NoStackTraceException("服务端不支持浏览器渲染(startBrowser),请在手机版阅读APP中使用或更换书源")
    }

    fun startBrowser(urlStr: String, headerStr: String): String =
        startBrowser(urlStr, headerStr, null)

    fun startBrowser(urlStr: String): String =
        startBrowser(urlStr, "", null)

    /**
     * 当前浏览器页面 URL,服务端无浏览器返回空
     */
    ...
    /**
     * 浏览器 cookie,服务端无浏览器返回空
     */
    fun getBrowserCookies(tag: String?): String {
        return ""
    }

    fun getBrowserCookies(): String {
        return ""
    }

    /**
     * WebView 默认 UA,返回常见移动端 UA 供书源 header 使用
     */
    fun getWebViewUA(): String {
        return "Mozilla/5.0 (Linux; Android 13; SM-S901U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.6422.165 Mobile Safari/537.36"
    }
