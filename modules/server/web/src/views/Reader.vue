      ...
      <div class="float-btn-zone">
        ...
        <div class="float-right-btn-zone">
          ...
          <div class="float-btn" :style="popupAbsoluteBtnStyle" @click="showReadBar = !showReadBar"
            v-if="speechAvalable && voiceList.length > 0 && !isEpub && !isCarToon && !isAudio">
...
export default {
  ...
  computed: {
    ...
    readBarTheme() {
      return {
        ...
        zIndex: 200,
        display: this.speechAvalable && this.voiceList.length > 0 && this.showReadBar ? "block" : "none",
  ...
  methods: {
    ...
    fetchVoiceList() {
      ...
      // 语音列表就绪后,若用户尚未选择语音,自动选中第一个(优先中文),避免点击播放静默失败
      if (!this.voiceName && this.voiceList.length > 0) {
        const zh = this.voiceList.find(v => v.lang && v.lang.toLowerCase().startsWith("zh-"));
        this.voiceName = (zh || this.voiceList[0]).name;
      }
    },
    ...
    startSpeech() {
      ...
      if (!this.voiceName) {
        this.$message.info("请先在语音库中选择一个语音");
        return;
      }
      const voice = this.voiceList.find(v => v.name === this.voiceName);
      if (!voice) {
        this.$message.info("当前语音不可用,请重新选择语音库");
        return;
      }
      ...
      this.utterance.onstart = () => {
        ...
        // Chrome 在长文本(约 15s)后会自行停止,定时 pause/resume 续命
        this.clearSpeechKeepAlive();
      ...
      this.utterance.onerror = event => {
        this.clearSpeechKeepAlive();
        // stopSpeech() 主动 cancel() 会触发 onerror(error=interrupted/canceled),
        // 这是正常停止,不是真错误,静默处理即可
        const aborted =
          this.skipAutoNext ||
          event.error === "interrupted" ||
          event.error === "canceled";
        if (!aborted && (event.error || event.name)) {
          this.$message.error(
    ...
    speechPrev() {
      ...
      if (prev) {
        ...
        // 延时启动,等待 cancel() 后 speaking 标志清掉,否则 startSpeech 会被守卫拦截
        setTimeout(() => {
          this.startSpeech();
        }, 100);
      } else {
