  ...
  >
    <div class="custom-dialog-title" slot="title">
      <span class="el-dialog__title"
        ...
        <span class="float-right span-btn" @click="restoreDefault"
          >恢复默认</span
        >
        <input
          ref="fileRef"
    ...
    <div class="source-container table-container">
      ...
      >
        ...
        <el-table-column label="作用于" min-width="90px">
...
export default {
  ...
  methods: {
    ...
    formatScope(row) {
      const tags = [];
      if (row.scopeTitle) tags.push("标题");
      if (row.scopeContent) tags.push("正文");
      return tags.length ? tags.join("/") : "—";
    },
    async restoreDefault() {
      const res = await this.$confirm(
        "确认导入内置默认替换规则(章节标题数字归一化)?将合并到现有规则,同名覆盖。",
        "提示",
        {
      ...
      Axios.get(this.api + "/getDefaultReplaceRules").then(
        res => {
          if (res.data.isSuccess) {
            ...
            if (ruleList.length) {
              ...
              this.$message.info("无默认替换规则");
            }
          }
        },
        error => {
          this.$message.error(
            "获取默认替换规则失败 " + (error && error.toString())
          );
        }
      );
    },
