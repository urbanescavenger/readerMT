...
class YueduApi : RestVerticle() {
    ...
    override suspend fun initRouter(router: Router) {
        ...
        router.get("/reader3/getDefaultReplaceRules").coroutineHandler { replaceRuleController.getDefaultReplaceRules(it) }
        router.post("/reader3/getDefaultReplaceRules").coroutineHandler { replaceRuleController.getDefaultReplaceRules(it) }
    ...
    suspend fun migration() {
        try {
            ...
            // 共享书源模式:首次启用时把各用户已有书源合并进共享池(default)
            migrateSharedBookSource()
        } catch(e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    /**
     * 共享书源模式下,首次启用时把各用户已有的 bookSource.json 按 bookSourceUrl 去重合并进 default 共享池。
     * 用标记文件 .bookSourceSharedMigrated 防止重复迁移。
     */
    private fun migrateSharedBookSource() {
        try {
            if (!appConfig.secure || !appConfig.sharedBookSource) {
                return
            }
            var defaultDir = File(getWorkDir("storage", "data", "default"))
            if (!defaultDir.exists()) {
                defaultDir.mkdirs()
            }
            var marker = File(defaultDir, ".bookSourceSharedMigrated")
            if (marker.exists()) {
                return
            }
            // 现有共享池
            var mergedArray = asJsonArray(getStorage("data", "default", "bookSource")) ?: JsonArray()
            var existedUrls = mutableMapOf<String, JsonObject>()
            for (i in 0 until mergedArray.size()) {
                var obj = mergedArray.getJsonObject(i)
                var url = obj.getString("bookSourceUrl", "") ?: ""
                if (url.isNotEmpty()) {
                    existedUrls.put(url, obj)
                }
            }
            // 遍历各用户目录合并
            var dataRoot = File(getWorkDir("storage", "data"))
            if (dataRoot.exists()) {
                dataRoot.listFiles { f -> f.isDirectory }?.forEach { userDir ->
                    var username = userDir.name
                    if (username.equals("default")) {
                        return@forEach
                    }
                    var userSource = asJsonArray(getStorage("data", username, "bookSource"))
                    if (userSource == null) {
                        return@forEach
                    }
                    for (i in 0 until userSource.size()) {
                        var obj = userSource.getJsonObject(i)
                        var url = obj.getString("bookSourceUrl", "") ?: ""
                        if (url.isNotEmpty() && !existedUrls.containsKey(url)) {
                            existedUrls.put(url, obj)
                            mergedArray.add(obj)
                        }
                    }
            ...
            saveStorage("data", "default", "bookSource", value = mergedArray)
            marker.createNewFile()
            logger.info("migrateSharedBookSource done, total: {}", mergedArray.size())
        } catch(e: Exception) {
            logger.error("migrateSharedBookSource error", e)
        }
    }
