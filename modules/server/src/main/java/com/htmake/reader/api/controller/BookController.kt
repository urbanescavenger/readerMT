...
class BookController(coroutineContext: CoroutineContext): BaseController(coroutineContext) {
    ...
    suspend fun getChapterList(context: RoutingContext): ReturnData {
        ...
        // 应用作用于标题的用户替换规则(如 #01 数字标题#JS 章节标题归一化)
        chapterList = applyTitleReplaceRules(chapterList, bookInfo, userNameSpace)

        return returnData.setData(chapterList)
    }

    /**
     * 对章节列表的标题应用用户替换规则(scopeTitle)。
     * 仅 scopeTitle=true 且 isEnabled 且在 scope 内的规则生效;无此类规则时直接返回原列表(零开销)。
     * 支持 @js: / <js></js> 替换(复用引擎 RhinoScriptEngine,绑定 result=原标题);否则按正则/字面替换。
     * 非破坏式:不修改磁盘缓存,只改返回结果。
     */
    private fun applyTitleReplaceRules(chapterList: List<BookChapter>, book: Book, userNameSpace: String): List<BookChapter> {
        if (chapterList.isEmpty()) return chapterList
        val rules = getEffectiveTitleRules(book, userNameSpace)
        if (rules.isEmpty()) return chapterList
        return chapterList.map { ch ->
            var t = ch.title
            for (r in rules) {
                t = applyRuleToTitle(r, t, book)
            }
            if (t != ch.title) ch.apply { title = t } else ch
        }
    }

    private fun getEffectiveTitleRules(book: Book, userNameSpace: String): List<ReplaceRule> {
        var arr = asJsonArray(getUserStorage(userNameSpace, "replaceRule"))
        val list: List<ReplaceRule> = if (arr != null) {
            (0 until arr.size()).mapNotNull { i ->
                runCatching { arr.getJsonObject(i).mapTo(ReplaceRule::class.java) }.getOrNull()
            }
        } else {
            // 用户无替换规则文件,使用内置默认规则(如 #01 数字标题#JS)
            DefaultData.replaceRules
        }
        return list.filter { it.isEnabled && it.scopeTitle && inScope(it, book) }
    }

    private fun inScope(rule: ReplaceRule, book: Book): Boolean {
        val scope = rule.scope?.trim() ?: ""
        if (scope.isNotEmpty()) {
            val items = scope.split(",", "\n", " ").map { it.trim() }.filter { it.isNotEmpty() }
            if (items.isNotEmpty() && items.none { it == book.name || it == book.origin }) return false
        }
        val ex = rule.excludeScope?.trim() ?: ""
        if (ex.isNotEmpty()) {
            val items = ex.split(",", "\n", " ").map { it.trim() }.filter { it.isNotEmpty() }
            if (items.any { it == book.name || it == book.origin }) return false
        }
        return true
    }

    private fun applyRuleToTitle(rule: ReplaceRule, title: String, book: Book): String {
        if (rule.pattern.isNotEmpty()) {
            val matched = if (rule.isRegex) {
                runCatching { rule.pattern.toRegex().containsMatchIn(title) }.getOrDefault(false)
            } else {
        ...
        // @js: / <js></js> 替换
        val m = AppPattern.JS_PATTERN.matcher(rule.replacement)
        return if (m.find()) {
            ...
            title.replace(rule.pattern, rule.replacement)
        }
    }
    ...
    fun loadBookSourceStringList(userNameSpace: String, bookSourceGroup: String = ""): List<String> {
        // 共享书源模式下,主书源池全局使用 default 命名空间
        val ns = if (appConfig.secure && appConfig.sharedBookSource) "default" else userNameSpace
        var bookSourceList: JsonArray? = asJsonArray(getUserStorage(ns, "bookSource"))
