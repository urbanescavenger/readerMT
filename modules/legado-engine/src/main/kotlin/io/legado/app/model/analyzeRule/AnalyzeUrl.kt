...
/**
 * 引擎版 AnalyzeUrl(从 readerMT `model/analyzeRule/AnalyzeUrl.kt` 移植为平台无关)。
 *
 * **剥离**:`android.*`/`androidx.*`/`com.script.*`/glide/media3;
 * **SPI 改写**:`com.script.RhinoScriptEngine`→`Platform.rhino`、`BackstageWebView`→
 * `Platform.webView.renderHtmlWithJs`、`getShareScope`→`Platform.rhino.getOrCreateSharedScope`、
 * `AppConfig.*`→`Platform.appConfig`、`Debug.log`→`log`、`appDb` 无(经 CookieStore/CacheManager)、
 * `Book`→`BaseBook`、`ConcurrentRateLimiter(source)`→`ConcurrentRateLimiter(key, concurrentRate)`。
 * **删除**:`getMediaItem()`/`getGlideUrl()`(app-only 播放器/图片加载)、嵌套 `ConcurrentRecord`
 * (已抽顶层 `help/ConcurrentRecord`)、Cronet 分支(`AppConfig.isCronet`,app-only)。
 *
 * 与 app 同名同包不同模块(switchover 前 app 用自家 AnalyzeUrl,无 classpath 冲突)。
 * com.script→Platform.rhino、真WebView→Platform.webView 是 §3.3 最高风险 seam,语义盲定,
 * 错靠 §5c parity 兜底。
 */
...
class AnalyzeUrl(
    ...
    private val concurrentRateLimiter = ConcurrentRateLimiter(source?.getKey(), source?.concurrentRate)
    ...
    private fun replaceKeyPageJs() { //先替换内嵌规则再替换页数规则，避免内嵌规则中存在大于小于号时，规则被切错
        ...
        if (ruleUrl.contains("{{") && ruleUrl.contains("}}")) {
            ...
            val url = analyze.innerRule("{{", "}}") {
                ...
                when (jsEval) {
                    is String -> jsEval
                    is Double if jsEval % 1.0 == 0.0 -> String.format(Locale.ROOT, "%.0f", jsEval)
    ...
    fun evalJS(jsStr: String, result: Any? = null): Any? {
        ...
        bindings["book"] = ruleData as? BaseBook
        bindings["source"] = source
        bindings["result"] = result
        bindings["infoMap"] = infoMap
        val sharedScope = source?.let {
            Platform.rhino.getOrCreateSharedScope(it.getKey(), it.jsLib, coroutineContext)
        }
    ...
    fun put(key: String, value: String): String {
        if (key == "bookName" || key == "title") {
            log("≡变量 $key 在特定情况下会被覆盖，建议使用其他键名")
        }
    ...
    fun get(key: String): String {
        when (key) {
            "bookName" -> (ruleData as? BaseBook)?.let {
                return it.name
            }
                ...
                strResponse = when (method) {
                    RequestMethod.POST -> {
                        ...
                        Platform.webView.renderHtmlWithJs(
                            ...
                            result = null
                        ).let { StrResponse(res.url, it) }
                    }

                    else -> Platform.webView.renderHtmlWithJs(
                        url = url,
                        html = "",
                        javaScript = webJs ?: jsStr ?: "",
                        headerMap = headerMap,
    ...
    private fun getByteArrayIfDataUri(): ByteArray? {
        ...
        if (dataUriFindResult != null) {
            ...
            val byteArray = EncoderUtils.base64DecodeToByteArray(dataUriBase64, 0)
            return byteArray
        }
        return null
    }
    ...
    fun getUserAgent(): String {
        return headerMap.get(UA_NAME, true) ?: Platform.appConfig.userAgent
    }
