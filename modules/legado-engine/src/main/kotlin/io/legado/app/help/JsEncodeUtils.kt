...
/**
 * 引擎版 JsEncodeUtils(书源 JS 加解密 API 表面,计划 §3.3 / §9.4 / §9.5 big-bang 加密簇)。
 *
 * 对应 readerMT `app/.../help/JsEncodeUtils.kt` 的 `interface JsEncodeUtils`(517 行):
 * 纯加密方法(md5/AES/DES/3DES/digest/HMac/createSymmetricCrypto/createAsymmetricCrypto/
 * createSign),均为默认方法。
 *
 * 与 app 的差异:
 * - 去 `@JavascriptInterface`(android.webkit JS 桥注解;引擎用 RhinoEngine 绑定,不需要)。
 * - `android.util.Base64` → 引擎 `EncoderUtils`(java.util.Base64 + android flag 值映射;
 *   `Base64.NO_WRAP` → `EncoderUtils.NO_WRAP`,parity 留 §5c)。
 * - android crypto 助手 `help.crypto.{AsymmetricCrypto,Sign,SymmetricCryptoAndroid}` 已随本簇
 *   移植入引擎(去 `@Keep`),背靠 hutool-crypto(已加 `libs.hutool.crypto`)。
 *
 * 继承:`interface JsExtensions : JsEncodeUtils`(引擎),`interface BaseSource : JsExtensions`(待)。
 */
@Suppress("unused")
interface JsEncodeUtils {
    ...
    fun aesDecodeArgsBase64Str(
        ...
        return createSymmetricCrypto(
            "AES/${mode}/${padding}",
            EncoderUtils.base64DecodeToByteArray(key, EncoderUtils.NO_WRAP),
            EncoderUtils.base64DecodeToByteArray(iv, EncoderUtils.NO_WRAP)
        ).decryptStr(data)
    }
...
//******************消息摘要/散列消息鉴别码************************//
    ...
    fun digestBase64Str(
        ...
        return EncoderUtils.base64Encode(
            DigestUtil.digester(algorithm).digest(data), EncoderUtils.NO_WRAP
        )
    }
    ...
    fun HMacBase64(
        ...
        return EncoderUtils.base64Encode(
            HMac(algorithm, key.toByteArray()).digest(data), EncoderUtils.NO_WRAP
        )
    }
