...
/**
 * 引擎版 AnalyzeRule(从 readerMT `model/analyzeRule/AnalyzeRule.kt` 移植为平台无关)。
 *
 * **剥离**:`android.text.TextUtils`→`StringUtils.isEmpty`、`@Keep`、`com.script.*`→
 * `Platform.rhino`、`BackstageWebView`→`Platform.webView.renderHtmlWithJs`、
 * `isMainThread`→`Platform.isMainThread()`、`getShareScope`→`Platform.rhino.getOrCreateSharedScope`、
 * `Debug.log`→`log`、`WebBook`→`Platform.webBook`、`Book`→`BaseBook`。
 * **保留**:`org.mozilla.javascript.NativeObject`/`Scriptable`(引擎直连 rhino,§3.3)、
 * `org.apache.commons.text.StringEscapeUtils`(commons-text)、`topScopeRef`/`evalJSCallCount`
 * 共享作用域二级缓存(原型链语义,parity 敏感)。
 *
 * `reGetBook`/`refreshTocUrl` 经 `Platform.webBook` SPI 反调平台 WebBook(app 真实现,
 * 服务端 NoOp)。与 app 同名同包不同模块,switchover 前无冲突。
 */
...
class AnalyzeRule(
    ...
    fun evalJS(jsStr: String, result: Any? = null): Any? {
        ...
        val topScope = source?.let {
            Platform.rhino.getOrCreateSharedScope(it.getKey(), it.jsLib, coroutineContext)
        } ?: topScopeRef?.get()
        val scope = if (topScope == null) {
            (Platform.rhino.getRuntimeScope(bindings) as Scriptable).apply {
                if (evalJSCallCount++ > 16) {
                    topScopeRef = WeakReference(this.prototype)
                }
            }
        } else {
            bindings.apply {
    ...
    fun reGetBook() {
        ...
        runBlocking(coroutineContext) {
            withTimeout(1800000) {
                Platform.webBook.preciseSearchAwait(bookSource, book.name, book.author)
                    .getOrThrow().let {
                ...
                Platform.webBook.getBookInfoAwait(bookSource, book, false)
            }
        }
    }
