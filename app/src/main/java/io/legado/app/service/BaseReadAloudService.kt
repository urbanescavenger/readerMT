...
//            .putLong(MediaMetadataCompat.METADATA_KEY_DURATION, nowSpeak.toLong())
    ...
    override fun onAudioFocusChange(focusChange: Int) {
        if (AppConfig.ignoreAudioFocus) {
            AndroidAppLog.put("忽略音频焦点处理(TTS)")
            return
        }
        when (focusChange) {
            AudioManager.AUDIOFOCUS_GAIN -> {
                if (needResumeOnAudioFocusGain) {
                    AndroidAppLog.put("音频焦点获得,继续朗读")
                    resumeReadAloud()
                } else {
                    AndroidAppLog.put("音频焦点获得")
                }
            }

            AudioManager.AUDIOFOCUS_LOSS -> {
                AndroidAppLog.put("音频焦点丢失,暂停朗读")
                pauseReadAloud()
            }

            AudioManager.AUDIOFOCUS_LOSS_TRANSIENT -> {
                AndroidAppLog.put("音频焦点暂时丢失并会很快再次获得,暂停朗读")
                if (!pause) {
            ...
            AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK -> {
                ...
                AndroidAppLog.put("音频焦点短暂丢失,不做处理")
            }
        }
    }
    ...
    private fun upReadAloudNotification() {
        upNotificationJob = execute {
            try {
                ...
                AndroidAppLog.put("创建朗读通知出错,${e.localizedMessage}", e, true)
            }
        }
    }
    ...
    open fun nextChapter() {
        ReadBook.upReadTime()
        AndroidAppLog.putDebug("${ReadBook.curTextChapter?.chapter?.title} 朗读结束跳转下一章并朗读")
        resumeReadAloudInternal()
            ...
            when (state) {
                TelephonyManager.CALL_STATE_IDLE -> {
                    if (needResumeOnCallStateIdle) {
                        AndroidAppLog.put("来电结束,继续朗读")
                        resumeReadAloud()
                    } else {
                        AndroidAppLog.put("来电结束")
                    }
                }

                TelephonyManager.CALL_STATE_RINGING -> {
                    if (!pause) {
                        AndroidAppLog.put("来电响铃,暂停朗读")
                        needResumeOnCallStateIdle = true
                        pauseReadAloud()
                    } else {
                        AndroidAppLog.put("来电响铃")
                    }
                }

                TelephonyManager.CALL_STATE_OFFHOOK -> {
                    AndroidAppLog.put("来电接听,不做处理")
                }
            }
        }
    }
