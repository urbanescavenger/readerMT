...
import io.legado.app.model.analyzeRule.getMediaItem
...
class AudioPlayService : BaseService(),
    ...
    override fun onPlayerError(error: PlaybackException) {
        ...
        AndroidAppLog.put(errorMsg, error)
        toastOnUi(errorMsg)
    }
    ...
    override fun onAudioFocusChange(focusChange: Int) {
        if (AppConfig.ignoreAudioFocus) {
            AndroidAppLog.put("忽略音频焦点处理(有声)")
            return
        }
        when (focusChange) {
            AudioManager.AUDIOFOCUS_GAIN -> {
                if (needResumeOnAudioFocusGain) {
                    AndroidAppLog.put("音频焦点获得,继续播放")
                    resume()
                } else {
            ...
            AudioManager.AUDIOFOCUS_LOSS -> {
                AndroidAppLog.put("音频焦点丢失,暂停播放")
                pause()
            }

            AudioManager.AUDIOFOCUS_LOSS_TRANSIENT -> {
                AndroidAppLog.put("音频焦点暂时丢失并会很快再次获得,暂停播放")
                if (!pause) {
    ...
    private fun upAudioPlayNotification() {
        upNotificationJob = execute {
            try {
                ...
                AndroidAppLog.put("创建音频播放通知出错,${e.localizedMessage}", e, true)
            }
        }
    }
