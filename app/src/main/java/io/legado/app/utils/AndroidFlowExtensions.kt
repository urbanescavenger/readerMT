...
/**
 * Android 专属 Flow 并行/生命周期扩展 —— 引擎副本删除时误删的 `utils/FlowExtensions.kt` 的
 * app-only 部分。引擎 `FlowExtensions.kt` 已含 [mapAsync],故此处**不含** mapAsync(避免
 * 同包同名顶层函数冲突),其余 app 专属扩展全部恢复于此。
 */
